...
def create_object_group_dict(input_data,input_dict_1):
    ...
    for line in input_data_new:
        ...
        if "port-object" in line:
            if split_line[1] == "eq":
                match_port = re.search(regex_port, split_line[2])
                if match_port:
...
#def gen_xml_with_objects(object_result_dict):
   

if __name__ == "__main__":
    asa_output = open_asa_config("101-asa434._onlyobjtxt")
    #asa_output = open_asa_config("asa_short.txt")
    result_obj_net = create_object_network_dict(asa_output)
    #result_obj_gr = create_object_group_dict(asa_output,result_obj_net)
    #pprint(result_obj_net, width=200)
    #pprint(result_obj_gr, width=200)
    yaml_file_obj = "result_object_dict.yaml"   
    with open (yaml_file_obj, "w") as yaml_write:
        yaml.dump(result_obj_net, yaml_write)
    #with open (yaml_file_obj) as yaml_read:
    #    object_dict = yaml.safe_load(yaml_read)
    env = Environment(loader=FileSystemLoader("templates"),trim_blocks=True, lstrip_blocks=True)
    template = env.get_template("xml_object_template.txt")
    with open("result_object.xml", "w") as result_obj_xml:
